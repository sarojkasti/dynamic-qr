...
pub struct NepalPaySettings {
    ...
    pub user_id: String,
    /// PKCS#8 PEM private key content — used to RSA-sign the token string required by the NPI QR API.
    /// Optional: if empty, token signing is skipped.
    #[serde(alias = "private_key_path", default)]
